...
switch (toLower _side) do {
	...
	case "independent": {
		...
		_lookup = INDEP_FACTION_DICT;
	};
	case "west": {
		_availableFactions = [
			"Vanilla - NATO",
			"Vanilla - NATO (Pacific)",
			"@3CB BAF - Army - Arctic",
			"@3CB BAF - Army - Desert",
			"@3CB BAF - Army - MTP",
			...
			"@CUP - British Armed Forces",
			"@CUP - Bundeswehr",
			"@CUP - United States Army",
			"@IFA3 - Wehrmacht",
			"@OPTRE - UNSCDF",
			"@Project OPFOR - Afghan National Army",
			"@Project OPFOR - Iraqi Armed Forces",
			"@RHS USAF  - USA Army - Desert",
			"@RHS USAF  - USA Army - Woodland"
		];

		_lookup = WEST_FACTION_DICT;
	};
	default {};
};
