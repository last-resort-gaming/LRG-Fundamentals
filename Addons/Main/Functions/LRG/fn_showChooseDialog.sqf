...
Author:
    Anton Struyk, modified by Kex & Mokka
*/
...
createDialog "LRG_Dynamic_Dialog";
...
private _tot_height = _row_heights call LR_fnc_sum;
...
private _titleText_varName = _titleText call LR_fnc_TextToVariableName;
private _titleVariableIdentifier = format ["LRG_ChooseDialog_DefaultValues_%1", _titleText_varName];
{
    _x params ["_choiceName", "_choices", ["_defaultChoice", 0], ["_force_default", false, [false]]];

	private _choiceName_varName = _choiceName call LR_fnc_TextToVariableName;
	...
	if (_choices isEqualType []) then
	{
		...
		if (_ResourceScript != "") then
		{
			...
			uiNamespace setVariable [format["LRG_ChooseDialog_ReturnValue_%1",_forEachIndex], _defaultChoice];
		...
		} else
		{
			...
			_comboScript = "uiNamespace setVariable [format['LRG_ChooseDialog_ReturnValue_%1'," + str (_forEachIndex) + "], _this select 1];"
		};
	...
	else
	{
		...
		} else
		{
			...
			private _ctrl_type = switch (_choices) do
			{
				...
				case "MESSAGE": {"RscLRGMessageEdit"};
				default {"RscLRGEdit"};
			};
			...
			if (_choices == "SLIDER") then
			{
				...
				_ctrl ctrlSetEventHandler ["KeyUp", "uiNamespace setVariable [format['LRG_ChooseDialog_ReturnValue_%1'," + str (_forEachIndex) + "], sliderPosition (_this select 0)];"];
				_ctrl ctrlSetEventHandler ["MouseButtonUp", "uiNamespace setVariable [format['LRG_ChooseDialog_ReturnValue_%1'," + str (_forEachIndex) + "], sliderPosition (_this select 0)];"];
			...
			} else
			{
				...
				_ctrl ctrlSetEventHandler ["KeyUp", "uiNamespace setVariable [format['LRG_ChooseDialog_ReturnValue_%1'," + str (_forEachIndex) + "], ctrlText (_this select 0)];"];
			};
			_ctrl ctrlCommit 0;
...
uiNamespace setVariable ["LRG_ChooseDialog_Result", -1];

if (_ResourceScript != "") then {call compile format["[""LOADED""] call %1;",_ResourceScript]};

LRG_var_showChooseDialog = true;
_dialog displayAddEventHandler ["unload",{LRG_var_showChooseDialog = nil}];
waitUntil { isNil "LRG_var_showChooseDialog" };
...
if (uiNamespace getVariable "LRG_ChooseDialog_Result" == 1) exitWith
{
	private _returnValue = [];
	{
		_returnValue pushBack (uiNamespace getVariable (format["LRG_ChooseDialog_ReturnValue_%1",_forEachIndex]));
	}forEach _choicesArray;
