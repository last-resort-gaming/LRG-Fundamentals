/*
Function: LR_fnc_TextToVariableName

Description:
    Replaces invalid letters in order to get a valid variable name

	Taken from Achilles (https://github.com/ArmaAchilles/Achilles)

Arguments:
	_input_unicode - Any kind of text
...
Author:
    Kex (based on KRON_Replace by Kronzky), modified by Mokka
*/


if (isNil "LR_var_old_special_char_unicode") then
{
	LR_var_old_special_char_unicode = [];
	LR_var_new_special_char_unicode = [];
	...
	{
		LR_var_old_special_char_unicode append (toArray (_old_letters select _i));
		LR_var_new_special_char_unicode append (toArray (_new_letters select _i));
	};
};
...
{
	private _letter_index = LR_var_old_special_char_unicode find (_input_unicode select _i);
	if (_letter_index != -1) then
	{
		_input_unicode set [_i,LR_var_new_special_char_unicode select _letter_index];
	};
};
