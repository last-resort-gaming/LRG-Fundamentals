class LRG_Dynamic_Dialog
{
	idd = 133798;
	movingEnable = true;
	
	class controls 
	{
		////////////////////////////////////////////////////////
		// GUI EDITOR OUTPUT START (by Zarg, v1.063, #Gocypi)
		////////////////////////////////////////////////////////

		class LRG_Title: RscText
		{
			idc = 1000;
			moving = 1;

			text = "Show Choose Dialog"; //--- ToDo: Localize;
			x = 2 * GUI_GRID_W_FIX + GUI_GRID_X;
			y = 0 * GUI_GRID_H_FIX + GUI_GRID_Y;
			w = 40 * GUI_GRID_W - 2 * GUI_GRID_W_FIX;
			h = 1.5 * GUI_GRID_H_FIX;
			size = TITLE_FONT_SIZE;
			sizeEx = TITLE_FONT_SIZE;
			colorBackground[] = {0.518,0.016,0,0.8};
		};
		class LRG_Main_Background: IGUIBack
		{
			idc = 2000;

			x = 0 * GUI_GRID_W + GUI_GRID_X;
			y = 1.5 * GUI_GRID_H_FIX + GUI_GRID_Y;
			w = 40 * GUI_GRID_W;
			h = 22.5 * GUI_GRID_H_FIX;
			colorBackground[] = {0.2,0.2,0.2,0.8};
		};
		class LRG_Content: RscControlsGroup
		{
			idc = 7000;
			x = 0 * GUI_GRID_W + GUI_GRID_X;
			y = 1.9 * GUI_GRID_H_FIX + GUI_GRID_Y;
			w = 40 * GUI_GRID_W;
			h = 22.5 * GUI_GRID_H_FIX;
		};
		class LRG_Dialog_Bottom_Bar: IGUIBack
		{
			idc = 2010;

			x = 1 * GUI_GRID_W + 6 * GUI_GRID_W_FIX + GUI_GRID_X;
			y = 22 * GUI_GRID_H_FIX + GUI_GRID_Y;
			w = 38 * GUI_GRID_W - 10 * GUI_GRID_W_FIX;
			h = 1.5 * GUI_GRID_H_FIX;
			colorBackground[] = {0,0,0,0.6};
		};
		class LRG_Ok_Button: RscButtonMenuOK
		{
			onButtonClick = "uiNamespace setVariable ['LRG_ChooseDialog_Result', 1]; closeDialog 1;";
			idc = 3000;
			x = 39.5 * GUI_GRID_W - 4 * GUI_GRID_W_FIX + GUI_GRID_X;
			y = 22 * GUI_GRID_H_FIX + GUI_GRID_Y;
			w = 4 * GUI_GRID_W_FIX;
			h = 1.5 * GUI_GRID_H_FIX;
			size = DEFAULT_FONT_SIZE;
			sizeEx = DEFAULT_FONT_SIZE;
			colorText[] = {1,1,1,1};
			colorBackground[] = {0,0,0,0.8};
		};
		class LRG_Cancle_Button: RscButtonMenuCancel
		{
			onButtonClick = "uiNamespace setVariable ['LRG_ChooseDialog_Result', -1]; closeDialog 2;";
			idc = 3010;
			x = 0.5 * GUI_GRID_W + GUI_GRID_X;
			y = 22 * GUI_GRID_H_FIX + GUI_GRID_Y;
			w = 6 * GUI_GRID_W_FIX;
			h = 1.5 * GUI_GRID_H_FIX;
			size = DEFAULT_FONT_SIZE;
			sizeEx = DEFAULT_FONT_SIZE;
			colorText[] = {1,1,1,1};
			colorBackground[] = {0,0,0,0.8};
		};
		class LRG_Icon_Background: IGUIBack
		{
			idc = 2020;

			x = 0 * GUI_GRID_W + GUI_GRID_X;
			y = 0 * GUI_GRID_H_FIX + GUI_GRID_Y;
			w = 2 * GUI_GRID_W_FIX;
			h = 1.5 * GUI_GRID_H_FIX;
			colorBackground[] = {0.518,0.016,0,0.8};
		};
		class LRG_Icon: RscPicture
		{
			idc = 2030;
			style = 48;
			text = "\z\LRG Fundamentals\addons\Media\images\LRG Logo.paa";
			x = 0.2 * GUI_GRID_W_FIX + GUI_GRID_X;
			y = 0.15 * GUI_GRID_H_FIX + GUI_GRID_Y;
			w = 1.6 * GUI_GRID_W_FIX;
			h = 1.2 * GUI_GRID_H_FIX;
		};
		////////////////////////////////////////////////////////
		// GUI EDITOR OUTPUT END
		////////////////////////////////////////////////////////

	};
};
