...
private _timeToCut = (_treeSize * _specialistFactor * "LRG_Logistics_axeTimeFactor") min ("LRG_Logistics_axeMaxTime") max 5;
...
private _onCompletion = {
	...
	[_treeObject] call LRG_fnc_checkCutDown;
	if !(_unit call ACE_common_fnc_isSwimming) then {
...
private _progressCheck = {
	...
	if (_passedTime > (_lastSoundEffectTime + SOUND_CLIP_TIME_SPACING)) then {
