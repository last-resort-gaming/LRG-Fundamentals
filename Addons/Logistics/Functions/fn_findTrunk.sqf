...
if (isNil "LRG_Logistics_trunkOffsetsCache") then {
	LRG_Logistics_trunkOffsetsCache = call CBA_fnc_createNamespace;
};
private _modelName = ((str _tree) splitString " .") select 1;
private _offset = LRG_Logistics_trunkOffsetsCache getVariable [_modelName, []];
...
LRG_Logistics_trunkOffsetsCache setVariable [_modelName,_offset];
